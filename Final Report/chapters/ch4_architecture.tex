...
\node[rectangle, draw=awsgray!50, fill=awslightorange, minimum width=\lw,
  ...
  (l5) {Layer 5 --- Generative Synthesis \quad \textcolor{awsgray}{\scriptsize Groq / Bedrock / Ollama (auto-fallback)}};

\node[rectangle, draw=awsgray!50, fill=awslightpurple, minimum width=\lw,
      minimum height=\lh, font=\small\bfseries, text centered,
      rounded corners=2pt, below=0pt of l5]
  (l4) {Layer 4 --- Hybrid Retrieval \& Fusion \quad \textcolor{awsgray}{\scriptsize filter $\to$ vector + BM25 $\to$ weighted RRF $\to$ cross-encoder}};
...
\caption{Six-layer conceptual architecture of TechPulse. Right-hand arrows show the upward data flow; the left-hand dashed arrow shows the query path. Layer~4 uses weighted BM25+RRF fusion with cross-encoder reranking, selected over weighted $\alpha$/$\beta$/$\gamma$ linear reranking based on design-phase analysis (Section~\ref{sec:sensitivity}). Layer~5 (Generative Synthesis) implements the LLM backend selection and fallback chain described in Section~\ref{sec:llm_fallback}.}
