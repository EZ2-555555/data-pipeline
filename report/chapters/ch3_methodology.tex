...
\textbf{True parallel hybrid retrieval with parameter-free fusion:} The system performs two independent retrieval passes---cosine vector search (pgvector) and BM25 keyword search (BM25Okapi over the full indexed corpus)---and fuses their rankings with Reciprocal Rank Fusion (RRF; Cormack et al., 2009). A third recency signal, computed as document age in days, is integrated as an additional RRF ranking. Because RRF operates on ordinal ranks rather than raw scores, it is parameter-free and scale-invariant: no weight tuning ($\alpha$/$\beta$/$\gamma$) is required, eliminating a common source of overfitting in hybrid retrieval systems. A design-phase comparison against weighted reranking ($\alpha$/$\beta$/$\gamma$) showed that weighted scoring is sensitive to corpus composition and incurs prohibitive tail latency (p95~=~25\,s); the empirical evidence favouring RRF is documented in Section~\ref{sec:sensitivity}. The BM25 index is built over the full corpus so that IDF statistics are globally accurate, and refreshed via a 30-minute TTL cache to capture newly ingested documents without rebuilding on every query.
...
RRF has two key advantages over weighted linear scoring: (1)~it is \emph{parameter-free}---no $\alpha$/$\beta$/$\gamma$ weights require tuning, eliminating a common source of overfitting; and (2)~it is \emph{scale-invariant}---because it operates on ordinal ranks, it handles the incomparable score distributions of cosine similarity ($[0,1]$) and BM25 ($[0, \infty)$) without normalisation. A design-phase comparison of weighted reranking (with parameters $\alpha$, $\beta$, $\gamma$) against RRF revealed that weighted scoring is sensitive to corpus composition and induces prohibitive tail latency (p95~=~25\,s), confirming RRF as the superior fusion strategy. Weighted reranking results appear only in design-phase analysis (Section~\ref{sec:sensitivity}); the Phase~B evaluation and final deployed system use RRF exclusively.

Figure~\ref{fig:retrieval_comparison} illustrates three retrieval strategies evaluated during development: the baseline vector-only pipeline (left), the weighted hybrid with dataset-dependent $\alpha$/$\beta$/$\gamma$ weights and p95~=~25\,s tail latency (centre, faded), and the final BM25+RRF hybrid pipeline selected for the production system (right).
