...
Generative model & HuggingFace / Ollama (fallback) & Multi-backend with auto-cascade; no Bedrock in Learner Lab \\
...
\midrule
1 & Amazon Bedrock (Claude Haiku) & $\to$ Ollama $\to$ HuggingFace & Not available in Learner Lab \\
...
Amazon Bedrock (Claude Haiku) & HuggingFace Inference API (Mistral-7B) + auto-fallback & Bedrock not available in Learner Lab & Same prompt template; evaluation comparable \\[4pt]
...
\begin{itemize}[leftmargin=0.5in]
  ...
  \item Integration with Amazon Bedrock for managed inference when access is available
