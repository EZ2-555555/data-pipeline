...
\caption{Retrieval strategies compared during design: baseline vector-only (left), weighted hybrid with $\alpha$/$\beta$/$\gamma$ reranking and p95~=~25\,s tail latency (centre, not selected), and weighted BM25+RRF hybrid pipeline with cross-encoder reranking selected for the production system (right).}
