...
Three considerations motivate the hybrid RAG approach. First, emerging technology knowledge is temporally sensitive; vector-only search lacks recency signals, whereas BM25+RRF fusion explicitly addresses temporal drift. Second, the system must operate within AWS Free Tier cost and latency bounds: full-corpus BM25 adds $O(N)$ computation against a cached index, and RRF merges ranked lists in $O(k)$, avoiding the prohibitive overhead of cross-encoder rerankers. Third, AWS-managed services (Lambda, RDS, API Gateway, EventBridge) enable a student team to deploy a production-grade system within the course timeline and budget.
...
\textbf{True parallel hybrid retrieval with parameter-free fusion:} The system performs two independent retrieval passes---cosine vector search (pgvector) and BM25 keyword search (BM25Okapi over the full indexed corpus)---and fuses their rankings with Reciprocal Rank Fusion (RRF; Cormack et al., 2009). A third recency signal is integrated as an additional RRF ranking. Because RRF operates on ordinal ranks, it is parameter-free and scale-invariant. A design-phase comparison against weighted reranking showed that weighted scoring is sensitive to corpus composition and incurs prohibitive tail latency; the empirical evidence is documented in Section~\ref{sec:sensitivity}.
...
RRF has two key advantages over weighted linear scoring: (1)~it is \emph{parameter-free}---no $\alpha$/$\beta$/$\gamma$ weights require tuning, eliminating a common source of overfitting; and (2)~it is \emph{scale-invariant}---because it operates on ordinal ranks, it handles the incomparable score distributions of cosine similarity ($[0,1]$) and BM25 ($[0, \infty)$) without normalisation. A design-phase comparison of weighted reranking against RRF revealed that weighted scoring is sensitive to corpus composition and induces prohibitive tail latency, confirming RRF as the superior fusion strategy (Section~\ref{sec:sensitivity}). The final deployed system uses RRF exclusively.
...
\caption{Retrieval strategies compared during design: baseline vector-only (left), weighted hybrid with $\alpha$/$\beta$/$\gamma$ reranking and p95~=~25\,s tail latency (centre, not selected), and BM25+RRF hybrid pipeline selected for the production system (right).}
