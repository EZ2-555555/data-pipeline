...
Evaluation Framework & Unit-tested & 50 queries, RAGAS, grid search, sensitivity analysis, statistical tests (code complete; full run pending) \\
...
Unit Tests & Integration-tested & 200 tests across 15 test files, all passing \\
\bottomrule
\end{tabular}
\end{table}
...
Several implementation decisions diverge from the original proposal due to AWS Free Tier constraints and practical engineering considerations. Table~\ref{tab:deviations} documents each deviation, its rationale, and the implemented alternative.
...
\midrule
Aurora Serverless + pgvector & RDS PostgreSQL db.t3.micro + pgvector & Aurora exceeds Free Tier budget & \textit{Trade-off:} No auto-scaling; db.t3.micro may bottleneck under high ingestion load. HNSW index and pgvector functionality preserved. \\[4pt]
Amazon Bedrock (Claude Haiku) & Groq (Llama 3.1 8B Instant) as primary + Bedrock (Amazon Nova Micro) as AWS-native fallback; Ollama / HuggingFace as local fallbacks & Groq free-tier API offers zero-cost inference & \textit{Trade-off:} Groq free tier has daily token limit (500k). Retrieval-layer findings remain backend-agnostic. \\[4pt]
2 data sources (ArXiv, Hacker News) & 5 data sources (+DEV.to, GitHub, RSS) & Improved corpus diversity and breadth & Enhanced retrieval coverage; additional maintenance surface for 3 ingesters. \\[4pt]
Custom VPC + NAT Gateway & Default VPC with security groups & Simplified initial deployment & \textit{Trade-off:} RDS publicly accessible; mitigated by security group restriction. Custom VPC migration identified as future improvement. \\[4pt]
AWS Budgets alerts & Cost Explorer programmatic check + AWS Budgets & Cost Explorer provides programmatic access in inference path & \textit{Trade-off:} Cost Explorer data lags by up to 24 hours; supplemented by AWS Budgets alerts for real-time governance. \\[4pt]
...
Hacker News every 5 minutes & Hacker News every 30 minutes & Rate limit considerations & Reduced API pressure; minor loss of near-real-time recency. \\[4pt]
...
The system includes 200 unit tests distributed across 15 test modules:
...
\texttt{test\_ingestion.py} & 24 & SHA-256 hash determinism and uniqueness for all 5 sources \\
\texttt{test\_retriever.py} & 9 & Keyword overlap computation + recency weight decay \\
...
\texttt{test\_db.py} & 9 & Database connection pooling + schema initialization \\
\texttt{test\_embedder.py} & 4 & Embedding model initialization + vector generation \\
\texttt{test\_http.py} & 3 & HTTP retry logic + session configuration \\
\texttt{test\_llm\_backends.py} & 15 & Multi-backend LLM fallback chain (Groq/Bedrock/HuggingFace/Ollama) \\
\texttt{test\_observability.py} & 27 & CloudWatch metrics + health checks + 5 drift simulation scenarios \\
\texttt{test\_pipeline.py} & 19 & End-to-end pipeline (chunk $\to$ embed $\to$ index) \\
\texttt{test\_queue.py} & 17 & SQS producer/consumer + DLQ behavior \\
\texttt{test\_scheduler.py} & 14 & Ingestion loop scheduling + error handling \\
\texttt{test\_storage.py} & 21 & S3 medallion layer operations \\
\texttt{test\_sync\_to\_aws.py} & 8 & AWS sync operations + configuration \\
\midrule
\textbf{Total} & \textbf{200} (passing) & \\
\bottomrule
\end{tabular}
\end{table}
...
Figure~\ref{fig:demo_tests} shows the terminal output of running the full test suite. All 200 tests pass across the 15 test modules, validating preprocessing, ingestion, retrieval, RAG orchestration, API endpoint behavior, LLM backends, observability, drift detection, and infrastructure integration.
...
\begin{itemize}
  ...
  \item The terminal must show \textbf{all 200 tests passing} (green dots or PASSED labels)
  \item The final summary line must read: \texttt{200 passed} (optionally with warnings)
...
\caption{Pytest execution: 200 unit tests passing across 15 test modules.}
...
\caption{pgAdmin: the \texttt{documents} table populated with records from all five data sources.}
\label{fig:demo_pgadmin}
\end{figure}

\medskip
\noindent This chapter documented implementation status across 20 components, deviations from the original proposal, and key engineering observations. All core pipeline components are integration-tested; cloud deployment is in progress. Chapter~7 presents remaining work and the project timeline.
