...
This project contributes at the intersection of data engineering, natural language processing, and MLOps in three ways. First, it provides controlled empirical evidence that hybrid retrieval (weighted BM25+RRF with cross-encoder reranking) measurably improves over vector-only baselines within a single-domain, cost-constrained setting---a configuration common in practice but under-represented in the literature. Second, it demonstrates that weighted Reciprocal Rank Fusion combined with a lightweight cross-encoder (ms-marco-MiniLM-L-6-v2) achieves a composite quality gain of $+0.047$ over the baseline while maintaining a manageable latency trade-off (mean $+1.36$\,s, Cohen's $d = -0.34$, small effect). Third, the end-to-end system integrates hybrid retrieval, drift detection, hallucination verification, and cost governance within a single deployable architecture on the AWS Free Tier, providing a practical blueprint for student and small-team RAG deployments.
...
2 & Hybrid vs.\ baseline retrieval precision & Context precision (RAGAS); faithfulness; citation grounding $> 0.80$ & Paired comparison over 50 queries; Wilcoxon signed-rank test; Cohen's $d$; bootstrap 95\% CI \\[4pt]
