...
\node[rectangle, draw=awsgray!50, fill=awslightpurple, minimum width=\lw,
  ...
  (l4) {Layer 4 --- Hybrid Retrieval \& RRF Fusion \quad \textcolor{awsgray}{\scriptsize filter $\to$ vector + BM25 $\to$ RRF}};
...
\caption{Six-layer conceptual architecture of TechPulse. Right-hand arrows show the upward data flow; the left-hand dashed arrow shows the query path. Layer~4 uses parameter-free BM25+RRF fusion, selected over weighted $\alpha$/$\beta$/$\gamma$ reranking based on design-phase analysis (Section~\ref{sec:sensitivity}).}
